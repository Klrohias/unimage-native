...
uint8_t *
tj3DecodeFromMemory(uint8_t *data, uint32_t size, int *width, int *height, int *channels, ErrorHandler *errorHandler) {
    ...
    try {
        ...
        if (!processor) {
            errorHandler->setErrorAndThrow(tjGetErrorStr());
        }
        ...
        if (tjDecompress2(processor, transformedData, transformedSize, buffer, *width, 0, *height, TJPF_RGB,
                          TJFLAG_FASTDCT) != 0) {
            errorHandler->setErrorAndThrow(tjGetErrorStr2(processor));
        }
